...
cudaError_t launch_imma_fp8_actquant_v3_f16(
  ...
  if (kChunk == 32) {
    ...
    imma_gemm_fp8_actquant_int8_colscale_fp16_colmajor_kernel_v3<32, half><<<grid, block, fp8imma_internal::smem_bytes_v2(32), stream>>>(
  ...
  if (kChunk == 64) {
    ...
    imma_gemm_fp8_actquant_int8_colscale_fp16_colmajor_kernel_v3<64, half><<<grid, block, fp8imma_internal::smem_bytes_v2(64), stream>>>(
        A_row_f16, B_col_fp8, col_scales_u16, D_col_f16, M, N, K, global_scale, a_inv_scale_h);
    return cudaGetLastError();
  }
...
cudaError_t launch_imma_fp8_actquant_v3_bf16(
  ...
  if (kChunk == 32) {
    (void)cudaFuncSetAttribute(
        ...
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        (int)fp8imma_internal::smem_bytes_v2(32));
    imma_gemm_fp8_actquant_int8_colscale_fp16_colmajor_kernel_v3<32, __nv_bfloat16><<<grid, block, fp8imma_internal::smem_bytes_v2(32), stream>>>(
  ...
  if (kChunk == 64) {
    (void)cudaFuncSetAttribute(
        ...
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        (int)fp8imma_internal::smem_bytes_v2(64));
    imma_gemm_fp8_actquant_int8_colscale_fp16_colmajor_kernel_v3<64, __nv_bfloat16><<<grid, block, fp8imma_internal::smem_bytes_v2(64), stream>>>(
        A_row_bf16, B_col_fp8, col_scales_u16, D_col_f16, M, N, K, global_scale, a_inv_scale_h);
    return cudaGetLastError();
  }
  return cudaErrorInvalidValue;
